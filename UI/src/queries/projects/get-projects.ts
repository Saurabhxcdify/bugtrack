...
export const GetProjects = (search: SearchParamsType) => {
  return infiniteQueryOptions({
    queryKey:  ["projects", searchParamsSerializer({ ...search, orderId: null })], // Include search params in the key to refetch on filter change
    queryFn: async ({ pageParam = 1 }) => {
      ...
      const response = await fetch(`/api/projects?page=${pageParam}&pageSize=${pageSize}${serialize}`);
