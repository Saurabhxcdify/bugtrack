...
export const GetUsers = (search: SearchParamsType) => {
  return infiniteQueryOptions({
    queryKey:  ["users", searchParamsSerializer({ ...search, orderId: null })], // Include search params in the key to refetch on filter change
    queryFn: async ({ pageParam = 1 }) => {
      ...
      const response = await fetch(`/api/users?page=${pageParam}&pageSize=${pageSize}${serialize}`);
