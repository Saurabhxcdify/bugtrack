...
import { Kbd } from "@/src/components/custom/kbd";
...
export function DataTableFilterCommand<TData, TSchema extends z.AnyZodObject>({
  ...
  const filterFields = useMemo(() => {
    return Object.keys(columnFilterSchema.shape).map((key) => {
      const field = columnFilterSchema.shape[key];
      return {
        value: key,
        label: key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, ' $1'), // Format label
        type: field._def.typeName, // Get the type from the schema
        commandDisabled: false, // Default value
      };
    });
  }, []);
      ...
      >
        ...
        {/* <div className="relative">
          <div className="absolute  top-2 z-10 w-full overflow-hidden rounded-lg border border-accent-foreground/30 bg-popover text-popover-foreground shadow-md outline-none animate-in">
            ...
            {/* <CommandList className="bg-red-200">
